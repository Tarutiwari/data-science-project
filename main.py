...
def melody_generator(model, seed_pool: np.ndarray, reverse_mapping: dict,
    ...
    # normalise any seed shape to a flat window of (length,) indices
    pool = np.asarray(seed_pool, dtype=np.float32)
    if pool.ndim > 1:
        pool = pool.reshape(pool.shape[0], -1)
    seed_flat = pool[np.random.randint(len(pool))].reshape(-1).copy()

    generated: list[int] = []

    for _ in range(note_count):
        pred = model(seed_flat[np.newaxis, :, np.newaxis], training=False)[0].numpy()
        ...
        seed_flat = np.concatenate([seed_flat[1:], new_token], axis=0)
...
def render_wav(midi_path: str, wav_path: str) -> None:
    if shutil.which("timidity") is None:
        print("timidity not found, skipping wav conversion")
        return
    subprocess.run(["timidity", midi_path, "-Ow", "-o", wav_path], check=False)
    print(f"WAV saved: {wav_path}")


def generate(note_count: int = 200, temperature: float = 0.8,
             midi_dir: str = DEFAULT_MIDI_DIR,
             out_dir: str = "outputs",
             length: int = 40,
             min_count: int = 100) -> str:
    weights_path = os.path.join(out_dir, "piano_lstm.weights.h5")

    if not os.path.exists(weights_path):
        return "Model weights not found. Please train the model first by running: python main.py"

    # rebuild corpus and mappings from midi files
    try:
        pieces = load_pieces(midi_dir)
    except SystemExit as e:
        return f"Error loading MIDI files: {e}"

    pieces, _ = filter_rare_pieces(pieces, min_count, min_len=length + 1)
    if not any(pieces.values()):
        return "No pieces survived filtering. Lower min_count."

    all_tokens = [t for ps in pieces.values() for p in ps for t in p]
    mapping, reverse_mapping, _, L_symb = build_mappings(all_tokens)
    scale = max(1, L_symb - 1)

    sequences = sequences_from_pieces(pieces, mapping, length)
    if set(sequences) == {"all"}:
        X_all, _ = sequences["all"]
    else:
        empty = empty_arrays(length)
        X_all, _ = sequences.get("test", sequences.get("train", empty))
    ...
    X_seed = scaled(X_all)
    if len(X_seed) == 0:
        return "Not enough data to generate from."

    model = build_model(length, L_symb)
    model.load_weights(weights_path)

    music_notes, melody = melody_generator(
        model, X_seed, reverse_mapping, scale, note_count, temperature
    )

    os.makedirs(out_dir, exist_ok=True)
    midi_path = os.path.join(out_dir, "Melody_Generated.mid")
    write_midi(melody, midi_path)

    return " ".join(str(n) for n in music_notes)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if set(sequences) == {"all"}:
        print("Flat folder detected, doing 80/10/10 split")
    ...
    if len(X_val) == 0:
        print("No validation data, using test set instead")
